...
wrapSession coherent_worker push_action pull_action = do
    ...
    forkIO $ runReaderT (outputGatherer push_action session_output ) framer_session_data 
...
addCapacity :: MonadIO m => 
        GlobalStreamId ->
        Int           -> 
        ReaderT FramerSessionData m ()
    ...
    if stream_id == 0 
      then 
        ...
        case val of 
            Nothing -> 
                -- ??
                liftIO $ putStrLn $ "Tried to update window of unexistent stream: " ++ (show stream_id)
...
inputGatherer :: PullAction -> SessionInput -> ReaderT FramerSessionData IO ()
inputGatherer pull_action session_input = do 
    ...
    let 
        source::Source (ReaderT FramerSessionData IO) B.ByteString
        source = transPipe liftIO $ F.readNextChunk http2FrameLength remaining pull_action
    ( source $$ consume)
  where 

    consume :: Sink B.ByteString (ReaderT FramerSessionData IO) ()
    consume = do 
        maybe_bytes <- await 
        -- Deserialize
...
outputGatherer :: PushAction -> SessionOutput -> ReaderT FramerSessionData IO ()
outputGatherer push_action session_output = do 
  ...
  where 
    ...
    pushFrame :: NH2.EncodeInfo
                 -> NH2.FramePayload -> ReaderT FramerSessionData IO ()
    pushFrame p1 p2 = do
    ...
    loopPart :: ReaderT FramerSessionData IO ()
    loopPart = do 
...
flowControlOutput :: Int -> (Chan FlowControlCommand) -> (Chan LB.ByteString) -> PushAction -> IO ()
flowControlOutput capacity commands_chan bytes_chan push_action = do 
    -- Get some bytes to send 
    bytes_to_send <- readChan bytes_chan
    if bytes_to_send == "" 
      then 
        -- This is a termination command 
        -- Check: somebody must destroy the channels....
        return ()
      else do
        -- Length?
        let amount = fromIntegral $ ((LB.length bytes_to_send) - 9)
        if  amount <= capacity 
          then do
            -- I can send 
            push_action bytes_to_send
            -- and tail-invoke 
            flowControlOutput (capacity - amount) commands_chan bytes_chan push_action
          else do
            -- I can not send, wait for a command instead 
            command <- readChan commands_chan
            case command of 
                AddBytes_FCM delta_cap -> 
                    flowControlOutput (capacity + delta_cap) commands_chan bytes_chan push_action