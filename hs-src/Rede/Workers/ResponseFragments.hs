...
simpleResponse :: Int -> B.ByteString -> TupledPrincipalStream
simpleResponse status_code text =
    let
        data_and_conclussion :: DataAndConclusion
        data_and_conclussion = do
            yield text
            return []
    in (trivialHeaders status_code (B.length text), [], data_and_conclussion)


ajaxResponseWithLength ::  Int -> Int -> B.ByteString -> B.ByteString -> TupledPrincipalStream
